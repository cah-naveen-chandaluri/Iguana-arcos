...
CASE WHEN PROD_841_D.PO_L.ITEM_NUM LIKE 'CF%' THEN substr(TRIM(PROD_841_D.PO_L.ITEM_NUM), 3)
	 ELSE PROD_841_D.PO_L.ITEM_NUM END
	 AS ITEM_NUM,
...
WHERE (
(PROD_841_D.PO_L.LAST_RECEIPT) >=  sysdate - 360) -- Duration should be 30 days in stage and production.
AND (PROD_841_D.PO_L.ORG_CODE in ('SB','AF','M1','CL','SE',
'VT','EC','MO','OX','RC','UC','JD','AO','OP','SC','PS','VI','AZ',
'ZG','IY','MD','AW','GB','WP','PN','HW','CR','MA','QN','BO','MP',
'BH','NI','UP','NT'))
